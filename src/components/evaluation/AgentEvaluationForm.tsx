...
const questions = [
  { key: "personaMatch" as const, label: "Persona Match", description: "Did it sound like the character?" },
  { key: "relevance" as const, label: "Relevance", description: "Were the roasts tailored to the opponent?" },
  { key: "funFactor" as const, label: "Fun Factor", description: "Did it make you smile or laugh?" },
  { key: "originality" as const, label: "Originality", description: "Did the roasts feel fresh, clever, and non-repetitive?" },
];
...
const AgentCard = ({ title, scores, onChange, variant }: AgentCardProps) => {
...
export const AgentEvaluationForm = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (mode === "human_vs_ai") {
      evaluations.push({
        participantId: "B",
        personality: agentAPersonality, // In human_vs_ai, AI is participant B
        scores: agentAScores,
      });
    } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="text-center">
        <h3 className="text-lg font-bold text-foreground mb-1">Rate the AI Agent{isAIvsAI ? "s" : ""}</h3>
      ...
      <div className={cn("gap-6", isAIvsAI ? "grid md:grid-cols-2" : "")}>
        <AgentCard
          title={isAIvsAI ? agentAName : agentAName}
          scores={agentAScores}
