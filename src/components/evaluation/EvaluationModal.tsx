  ...
  return <Dialog open={isOpen} onOpenChange={open => !open && onClose()}>
      <DialogContent className="w-full max-w-2xl bg-card border-border max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex gap-1 p-1 rounded-xl bg-muted mb-6">
          {tabs.map(tab => <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={cn("flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-lg text-sm font-medium transition-all", activeTab === tab.id ? "bg-card text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>
        ...
        <div className="min-h-[200px]">
          {activeTab === "scores" && evaluation && <div className="grid md:grid-cols-2 gap-4">
              <ScoreCard participantName={participantAName} scores={evaluation.participantAScores} isWinner={evaluation.winner === "A"} variant="primary" />
              <ScoreCard participantName={participantBName} scores={evaluation.participantBScores} isWinner={evaluation.winner === "B"} variant="secondary" />
              {evaluation.winner !== "TIE" && <div className="md:col-span-2 text-center py-4">
              ...
              {evaluation.threadText && <div className="md:col-span-2">
                  <TranscriptViewer threadText={evaluation.threadText} />
                </div>}
            </div>}

          {activeTab === "agent" && (agentFeedbackSubmitted ? <div className="flex flex-col items-center justify-center py-12">
                <div className="w-16 h-16 rounded-full bg-green-500/20 flex items-center justify-center mb-4">
                  <Bot className="w-8 h-8 text-green-400" />
                </div>
                <h3 className="text-xl font-bold text-foreground mb-2">Agent Rating Submitted!</h3>
                <p className="text-muted-foreground text-center">
                  Now rate the roast quality in "Your Vote" tab.
                </p>
              </div> : <AgentEvaluationForm mode={mode} agentAName={agentANameForEval} agentAPersonality={agentAPersonalityForEval || ""} agentBName={mode === "ai_vs_ai" ? participantBName : undefined} agentBPersonality={mode === "ai_vs_ai" ? participantBPersonality : undefined} onSubmit={handleAgentFeedbackSubmit} />)}

          {activeTab === "llm" && <div className="p-6 rounded-xl border border-border bg-muted/30">
          ...
          {activeTab === "feedback" && (feedbackSubmitted ? <div className="flex flex-col items-center justify-center py-12">
                ...
                <button onClick={onClose} className="px-6 py-2 rounded-lg border border-border text-foreground hover:bg-muted transition-colors">
                  View Battle Results
                </button>
              </div> : <FeedbackForm participantAName={participantAName} participantBName={participantBName} onSubmit={handleFeedbackSubmit} />)}
        </div>
      </DialogContent>
    </Dialog>;
};